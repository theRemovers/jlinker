...
let build_index ?(warn = fun _ -> ()) summary =
  ...
  in
  let f i (def_i, _undef_i) =
    Hashtbl.iter (fun name _ -> add_index name i) def_i
  in
...
let get_summary problem =
  let process_obj { Aout.symbols; filename; _ } =
    ...
    let add_defined name no =
      ...
      if Hashtbl.mem defined name then
        Log.warning "Symbol %s is ambiguous in object %s" name filename
  ...
  let f = function
    ...
    | Archive { Archive.content; filename; _ } ->
        ...
        let warn sym_name =
          Log.warning "Symbol %s multiply defined in archive %s" sym_name
            filename
        in
...
let solve problem =
  ...
  let mark_defined sym_name =
    ...
    if Hashtbl.mem defined_tbl sym_name then
  ...
  let mark_undefined sym_name =
    if Hashtbl.mem defined_tbl sym_name || Hashtbl.mem unresolved_tbl sym_name
    then ()
  ...
  let find_symbol sym_name =
    let rec aux = function
      | [] -> None
      | archno :: tl -> (
          match summary.(archno) with
          | `Object _ -> assert false
          | `Archive (def, objs) -> (
              match Hashtbl.find_opt def sym_name with
              | None -> aux tl
              | Some no -> Some (archno, no, objs.(no)) ) )
    in
    aux archives
  in
    ...
    let rec aux i =
      if i < n then
        ...
        | Archive { Archive.filename; content; _ }, `Archive (_, objs_sum) ->
              ...
              if j < n_objs then
                ...
                if Hashtbl.mem selected_tbl idx then
                  ...
                  ( {
                      obj with
                      Aout.filename =
                        filename ^ Filename.dir_sep ^ obj.Aout.filename;
                    },
                    obj_sum )
                  :: extract (j + 1)
                else extract (j + 1)
            ...
            in
            extract 0 @ aux (i + 1)
        | Object _, `Archive _ | Archive _, `Object _ -> assert false
      else []
    in
    Array.of_list (aux 0)
  in
  ...
  let unresolved_symbols =
    let f i (_def_i, undef_i) unresolved_symbols =
      ...
      let rec update_unresolved = function
        ...
        | ((sym_name, current_value) as sym) :: tl -> (
            match Hashtbl.find_opt undef_i sym_name with
    ...
    let symbols =
      ref
        (List.map
           (fun sym_name -> (sym_name, 0l))
           (HashtblExt.keys unresolved_tbl))
    in
